 ...
 * @since 1.0
 */
class ETabView extends CTabView
{
    /**
    ...
    {
        //$reset = reset($this->tabs);
        //$this->activeTab = key($reset);
    ...
    public function registerClientScript()
    {
        ...
        $assetsPath = $this->owner->module->getAssetsUrl();
        $cs->registerScriptFile($assetsPath . '/js/tabs.js');
    ...
    protected function renderHeader()
    {
        ...
        foreach($this->tabs as $id=>$tab)
        {
            if(isset($tab['ajax']) && ($tab['ajax'])){
                $ajax = ' data-ajax="true" data-div="'.$id.'"';
            }else{
                $ajax = '';
            }
            ...
            echo "<li><a id=\"{$id}-link\" {$ajax}href=\"{$url}\"{$active}>{$title}</a></li>\n";
        }
        echo "</ul>\n";
    }
